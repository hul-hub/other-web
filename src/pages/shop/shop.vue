		...
		<scroll-view :scroll-top="scrollTop" scroll-y="true" class="scroll-Y" @scrolltoupper="upper" @scrolltolower="lower"
		 @scroll="scroll">
			<view class="item-money-box" v-for="(item, index) in list" :index="index" :key="index" @click="goPersonDetail(item)">
				<view class="img-box">
					<!-- <view class="icon-img" :style="{'backgroundImage':'url('+item.url+')'}"> -->
					<image class="icon-img" :src="require('static/icon/shop2.png')" mode=""></image>
					<!-- </view> -->
				</view>
				<view class="text-box">
	...
	export default {
		...
		methods: {
			...
			goPersonDetail(item) {
				let that = this;
				uni.navigateTo({
					url: '../shopChild/shopChild?id=' + item.usercode +"&name="+item.username
				});
			},
			loadData() {
				var that = this;
